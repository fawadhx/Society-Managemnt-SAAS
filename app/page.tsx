  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      <div className="page-wrap">
        ...
        {view === 'Dashboard' ? <Dashboard trend={trend} setTrend={setTrend} open={open} /> : <SectionView view={view} query={query} filteredResidentRows={filteredResidentRows} paymentRows={paymentRows} open={open} />}
      </div>
    </main>
...
function Dashboard({ trend, setTrend, open }: { trend: 'collection' | 'outstanding'; setTrend: (v: 'collection' | 'outstanding') => void; open: (v: Exclude<Modal, null>) => void }) {
  const { payments: ctxPayments, overdueResidents: ctxOverdue, canBatchGenerate, canSendAutomatedReminders, currentTier, adminName, currentSociety } = useSociety()
  const paymentRows = useMemo(() => ctxPayments.map(p => [p.receiptId, p.residentName, p.unitNumber, fmt(p.amount), p.date, p.method]), [ctxPayments])
  ...
  return <><div className="page-heading"><div><p className="eyebrow">Wednesday, 20 August 2026</p><h1>Good morning, {adminName.split(' ')[0]}</h1><p className="muted">Here&apos;s what&apos;s happening with your society today.</p></div><div className="heading-actions"><Button variant="outline" disabled={!canSendAutomatedReminders} onClick={() => open('reminder')}><Send data-icon="inline-start" />Send reminder{!canSendAutomatedReminders && <span className="upgrade-hint">Upgrade to T2</span>}</Button><Button disabled={!canBatchGenerate} onClick={() => open('charges')}><Plus data-icon="inline-start" />Generate charges{!canBatchGenerate && <span className="upgrade-hint">Upgrade to T2</span>}</Button><span className="tier-current-badge">{currentTier === 'TIER_1' ? 'T1 Basic' : currentTier === 'TIER_2' ? 'T2 Pro' : 'T3 Enterprise'}</span></div></div>
    <div className="kpi-grid"><Kpi title="Total collection" value={fmt(524500)} change="12.8%" icon={CircleDollarSign} color="teal" /><Kpi title="Outstanding balance" value={fmt(186000)} change="8.4%" icon={WalletCards} color="amber" down /><Kpi title="Collection rate" value="73.8%" change="5.2%" icon={TrendingUp} color="blue" /><Kpi title="Overdue accounts" value={String(ctxOverdue.length)} change="3 fewer" icon={AlertCircle} color="red" down /></div>
    ...
    <div className="dashboard-grid"><section className="panel trend-panel"><div className="panel-head"><div><h2>Collection overview</h2><p>Monthly financial performance</p></div><div className="toggle"><button className={trend === 'collection' ? 'selected' : ''} onClick={() => setTrend('collection')}>Collected</button><button className={trend === 'outstanding' ? 'selected' : ''} onClick={() => setTrend('outstanding')}>Outstanding</button></div></div><div className="chart-meta"><strong>{trend === 'collection' ? fmt(524500) : fmt(186000)}</strong><span className="positive"><ArrowUpRight size={14} /> 12.8% vs last month</span></div><div className="chart"><div className="chart-y"><span>600k</span><span>400k</span><span>200k</span><span>0</span></div><div className="chart-area"><div className="grid-lines"><i /><i /><i /><i /></div><svg viewBox="0 0 700 220" preserveAspectRatio="none" aria-label="Collection trend chart"><defs><linearGradient id="fill" x1="0" x2="0" y1="0" y2="1"><stop offset="0" stopColor="var(--primary)" stopOpacity=".22" /><stop offset="1" stopColor="var(--primary)" stopOpacity="0" /></linearGradient></defs><path d="M0,166 C45,146 60,154 100,130 S170,145 200,112 S265,124 300,98 S360,115 400,84 S455,105 500,67 S565,89 600,49 S670,62 700,28 L700,220 L0,220 Z" fill="url(#fill)" /><path d="M0,166 C45,146 60,154 100,130 S170,145 200,112 S265,124 300,98 S360,115 400,84 S455,105 500,67 S565,89 600,49 S670,62 700,28" fill="none" stroke="var(--primary)" strokeWidth="3" /></svg><div className="chart-x"><span>Mar</span><span>Apr</span><span>May</span><span>Jun</span><span>Jul</span><span>Aug</span></div></div></div></section>
      <section className="panel breakdown"><div className="panel-head"><div><h2>Payment status</h2><p>August 2026 invoices</p></div><button className="icon-button"><MoreHorizontal size={18} /></button></div><div className="donut-wrap"><div className="donut"><div><strong>218</strong><span>Invoices</span></div></div><div className="legend"><Legend color="var(--primary)" label="Paid" value="73.8%" /><Legend color="var(--warning)" label="Partial" value="12.4%" /><Legend color="var(--danger)" label="Overdue" value="13.8%" /></div></div><div className="breakdown-foot"><span><CheckCircle2 size={15} />161 paid</span><span><Clock3 size={15} />27 pending</span></div></section></div>
    <div className="lower-grid"><section className="panel"><div className="panel-head"><div><h2>Recent payments</h2><p>Latest transactions across {currentSociety.name.split(' ')[0]} {currentSociety.name.split(' ')[1]}</p></div><Button variant="ghost" size="sm" onClick={() => open('payment')}>View all <ArrowUpRight data-icon="inline-end" /></Button></div><DataTable headers={['Receipt', 'Resident', 'Unit', 'Amount', 'Date', 'Method']} rows={paymentRows} /></section><section className="panel attention"><div className="panel-head"><div><h2>Needs attention</h2><p>Accounts that need follow-up</p></div><span className="count-badge">{ctxOverdue.length}</span></div>{ctxOverdue.slice(0, 3).map(item => <div className="attention-row" key={item.id}><div className="unit-avatar">{item.unitNumber.slice(0, 1)}</div><div className="attention-name"><strong>{item.name}</strong><span>{item.unitNumber} · {item.daysOverdue} days overdue</span></div><strong className="amount">{fmt(item.balance)}</strong><button className="small-send" onClick={() => open('reminder')} aria-label={`Remind ${item.name}`}><Send size={14} /></button></div>)}<Button variant="outline" className="full-button" onClick={() => open('reminder')}>View all reminders</Button></section></div>
  </>
}
...
function SectionView({ view, query, filteredResidentRows, paymentRows, open }: { view: View; query: string; filteredResidentRows: string[][]; paymentRows: string[][]; open: (v: Exclude<Modal, null>) => void }) {
  ...
  if (view === 'Reports') {
    return <><div className="page-heading"><div><p className="eyebrow">{currentSociety.name}</p>
      ...
      <div className="kpi-grid">
        <Kpi title="Total Invoiced" value={fmt(710500)} change="8.2%" icon={Receipt} color="teal" />
        <Kpi title="Total Collected" value={fmt(524500)} change="12.8%" icon={CircleDollarSign} color="blue" />
        <Kpi title="Total Outstanding" value={fmt(186000)} change="4.1%" icon={WalletCards} color="amber" down />
        <Kpi title="Collection Rate" value="73.8%" change="5.2%" icon={TrendingUp} color="teal" />
      </div>
  ...
  if (view === 'Reminders') {
    return <><div className="page-heading"><div><p className="eyebrow">{currentSociety.name}</p><h1>{c.title}</h1><p className="muted">{c.subtitle}</p></div><Button onClick={() => open('reminder')}><Plus data-icon="inline-start" />Send reminder</Button></div>
      ...
      <section className="panel list-panel">
        <div className="panel-head"><div><h2>Overdue residents</h2><p>{overdueResidents.length} accounts with outstanding balances</p></div></div>
        <div className="table-scroll"><table><thead><tr>{['Resident Name', 'Unit #', 'Outstanding Balance', 'Days Overdue', 'Last Reminder', 'Action'].map(h => <th key={h}>{h}</th>)}</tr></thead><tbody>
  ...
  const propertyRows = units.map(u => [u.unitNumber, u.type, `${currentSociety.name.split(' ')[0]} Block ${u.block}`, u.occupancy, fmt(u.monthlyCharge)])
  const rows = view === 'Residents' ? filteredResidentRows : view === 'Payments' ? paymentRows : propertyRows
  return <><div className="page-heading"><div><p className="eyebrow">{currentSociety.name}</p><h1>{c.title}</h1><p className="muted">{c.subtitle}</p></div>{c.action && <Button onClick={() => open(c.action!)}><Plus data-icon="inline-start" />{c.action === 'charges' ? 'Generate charges' : c.action === 'payment' ? 'Record payment' : c.action === 'reminder' ? 'Send reminder' : c.action === 'property' ? 'Add property' : 'Add resident'}</Button>}</div><div className="sub-kpis"><Kpi title="Total records" value={view === 'Residents' ? String(filteredResidentRows.length) : String(units.length)} change="6.2%" icon={view === 'Residents' ? Users : Building2} color="teal" /><Kpi title="Active / paid" value={view === 'Residents' ? String(filteredResidentRows.length) : fmt(524500)} change="12.8%" icon={CheckCircle2} color="blue" /><Kpi title="Needs attention" value={fmt(186000)} change="3.4%" icon={AlertCircle} color="amber" down /></div><section className="panel list-panel"><div className="panel-head"><div><h2>{view === 'Residents' ? 'Resident directory' : view === 'Payments' ? 'Payment history' : `${c.title} overview`}</h2><p>{query ? `Showing results for \u201c${query}\u201d` : 'Updated a few moments ago'}</p></div><div className="list-actions"><button className="filter-button"><SlidersHorizontal size={15} />Filters</button><button className="filter-button"><Download size={15} />Export</button></div></div><DataTable headers={view === 'Residents' ? ['Unit', 'Resident', 'Phone', 'Outstanding', 'Status'] : view === 'Payments' ? ['Receipt', 'Resident', 'Unit', 'Amount', 'Date', 'Method'] : ['Unit', 'Type', 'Location', 'Occupancy', 'Monthly charge']} rows={rows} /></section></>
}
